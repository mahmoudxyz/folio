...
- **The initial writing must be human.** The ideas, structure, explanations, and voice of every piece must come from a human author. Do not submit content where the core text was generated by an LLM and lightly edited — readers can tell, and it defeats the purpose of this project.
- **AI is fine for code.** Using AI tools (Copilot, Claude, ChatGPT, etc.) to write or assist with code — both in pieces and on the site itself — is perfectly welcome.
- **AI is fine for proofreading.** Using AI to check grammar, catch typos, improve clarity, or suggest better phrasing on content you already wrote is encouraged. The key distinction: AI as an editor, not as the author.
